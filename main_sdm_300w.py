...
def train_model(fpath, data, n_steps=5, savefile=None):
    ...
    model = CascadeMethod() if savefile is None else cPickle.load(open(savefile, 'rb'))
...
def validate_model(mpath, rpath, data, steps=None):
    ...
    predictions = model.align(data['images'], num_steps=steps, save_all=True, augmenter=augmenter, n_augs=25)
...
def evaluate(mpath, rpath, data):
    ...
    for preds in predictions[0]:
        n_inst, n_pars = len(data['images']), preds[0].shape[1]
        preds = np.mean(preds[0].reshape((-1, n_inst, n_pars)), axis=0)
        print "MEE: " + str(evaluate_results(model, data['images'], data['landmarks'], preds, lmk_l=[36, 39], lmk_r=[42, 45]))


if __name__ == '__main__':
    # Load data and partition indices
    model_file, results_file = ('sdm_300w.pkl', 'sdm_300w_results.pkl')
