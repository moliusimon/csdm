...
from cascaded.cascade.csdm_rotate import CascadeCsdmRotate as CascadeMethod
...
def evaluate(mpath, rpath, data):
    ...
    for preds in predictions[0]:
        ...
        print "MEE: " + str(evaluate_results(model, data['images'], data['landmarks'], preds, lmk_l=[0, 4], lmk_r=[8, 12]))


if __name__ == '__main__':
    # Load data and partition indices
    model_file, results_file = ('continuous_bu4dfep.pkl', 'continuous_bu4dfep_results.pkl')
    path = '/home/cvc/moliu/Datasets/bu4dfe+/'
    data = load_bu4dfep(path)

    # Train model, validate, evaluate results
    train_model(path+model_file, data['train'], n_steps=10, savefile=None)
